...
Usage:
    python tools/health.py --kb my-kb            # check specific KB
    python tools/health.py --kb kb1 --kb kb2     # check multiple KBs
    python tools/health.py                        # check all KBs
...
def check_index_sync(pages: list[Path], kb_name: str) -> dict:
    ...
    index_file = kb_paths["index_file"]
...
def run_health(kb_list: list[str]) -> dict:
    """Run all health checks for specified KBs, return structured results."""
    all_pages = []
    all_empty = []
    all_index_sync = {"in_index_not_on_disk": [], "on_disk_not_in_index": []}
    all_log_coverage = []
    ...
    for kb_name in kb_list:
        ...
        empty = check_empty_files(pages, kb_name)
        all_empty.extend(empty)

        index_sync = check_index_sync(pages, kb_name)
        all_index_sync["in_index_not_on_disk"].extend(index_sync["in_index_not_on_disk"])
        all_index_sync["on_disk_not_in_index"].extend(index_sync["on_disk_not_in_index"])

        log_coverage = check_log_coverage(pages, kb_name)
        all_log_coverage.extend(log_coverage)

    return {
        "date": date.today().isoformat(),
        "kbs_checked": kb_list,
        "total_pages": len(all_pages),
        "empty_files": all_empty,
        "index_sync": all_index_sync,
        "log_coverage": all_log_coverage,
    }
...
def format_report(results: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"KBs checked: {', '.join(results['kbs_checked'])}",
    ...
    lines.append("")
    if empty:
        lines.append("| KB | Page | Total Bytes | Body Bytes | Status |")
        lines.append("|---|---|---|---|---|")
        for ef in empty:
            emoji = "🔴" if ef["status"] == "empty" else "🟡"
            lines.append(f"| {ef['kb']} | `{ef['path']}` | {ef['total_bytes']} | {ef['body_bytes']} | {emoji} {ef['status']} |")
    else:
    ...
    if log_missing:
        ...
        for lm in log_missing:
            lines.append(f"- [{lm['kb']}] `{lm['path']}` — {lm['title']}")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kb", action="append",
                        help="Knowledge base name (can be specified multiple times). If not provided, checks all KBs.")
    ...
    if args.kb:
        ...
        if not kb_list:
            ...
            sys.exit(0)
        print(f"No KB specified, checking all: {kb_list}")

    results = run_health(kb_list)
