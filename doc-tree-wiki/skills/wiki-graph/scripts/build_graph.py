...
Usage:
    python tools/build_graph.py --kb my-kb             # build graph for specific KB
    python tools/build_graph.py --kb my-kb --open     # open in browser after build
    python tools/build_graph.py --no-infer             # skip semantic inference (faster)

Outputs:
    knowledge-base/{kb}/graph/graph.json    — node/edge data (cached by SHA256)
    knowledge-base/{kb}/graph/graph.html    — interactive vis.js visualization
